...
#define TLS_MODE_CMD_CTAG_NON   "c-tag-non"
...
struct ppe_meter_cfg {
    ...
    unsigned int retVal     :1; /* 0:success, 1: fail */
    unsigned int addFlowOnly:1; /* 0:add Meter & flow, 1: add flow only */
...
struct ppe_hash_cfg {
    ...
    unsigned int tlsEtypeDis    :1; /* 0:enable, 1:disable (including EthType and DSCP hash when TLS) */
    unsigned int tlsPbitsDis    :1; /* 0:enable, 1:disable (VLAN Pbits hash when TLS) */
    unsigned int tlsDmacDis     :1; /* 0:enable, 1:disable (DMAC hash when TLS) */
    unsigned int method         :4; /* command method from user */
