...
void sendInform(){
		...
		if(cwmpRepeatFlag && repeatTimes && repeatPeriodicVal){
			...
			tc_printf("\r\ncwmpInit");
				customTimerID = timerPeriodicStartS( repeatPeriodicVal * second,  (TimerCallback)cwmpPeriodInform, NULL );
...
int startPeriodicInform(void)
{
	...
	*/
	...
	{
		if(getInformPeriodEnable() && period && !informTimerID &&  !checkPeriodicInformTime()) 
			informTimerID = timerPeriodicStartS( period, (TimerCallback)cwmpPeriodInform, NULL );
	}
#else
...
#else
char devreg_devType[32] = "E8C 4+2 "
#endif
#endif
#endif
...
#if 1//
	tcdbg_printf("\r\nget_old_tcp_windows(): p1 = %u, p2 = %u, p3 = %u", p1, p2, p3);
#endif
