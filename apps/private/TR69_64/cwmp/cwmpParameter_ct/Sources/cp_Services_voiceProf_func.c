...
static uint32 _IADReasonGet(int instance)
{
    int    ret = 0;
    FILE   *fp = NULL;
    char   tempValue[16] = {0};
    ...
    fp = fopen(IAD_TEST_REASON_PATH, "r");
...
static int checkIADModule(int instance)
{
    ...
    /*judge pcm interrupt*/
    ret = _IADReasonGet(instance);
    if (-1 != ret && IAD_INNER_ERROR != ret)
        ret = 0;
...
static uint32 cwmpIADEmulateReg()
{
	...
	if (getSipStatus_tr69() == 0){
		...
		/* Megaco simulate */
		snprintf(cmd, sizeof(cmd)-1, "callCmd {test.IAD}{%d}", gCwmpIAD.testServer);
		cwmpVoIPCmdSend("127.0.0.1", 3555, cmd);
	}
#endif
...
void *cwmpIADTask(void)
{
    ...
    resetCwmpIADResult();//reset the IAD Result before ping.
    rpc.flags.theIADFlag &= ~CWMP_IAD_TERMINATED; 
...
int8 getRTPlpMarkValue(char** value, ParameterType* type, DeviceNode*  deviceNode)
{
	...
	tcapi_get(nodeName, "802-1pMark", tempValue);
...
{	
	...
	sprintf(setValue, "%d", val);
       tc_printf("\n==>nodeName:%s,setval:%s\n",nodeName,setValue);
	tcapi_set(nodeName, "802-1pMark", setValue);
	return 0;
}		
...
int8 getSIPlpMarkValue(char** value, ParameterType* type, DeviceNode*  deviceNode)
{
	...
	tcapi_get("VoIPBasic_Common", "802-1pMark", tempValue);
...
int8 setSIPlpMarkValue(void* value, struct DeviceNode*  deviceNode)
{
	...
	sprintf(setValue, "%d", ipmask);

	tcapi_set("VoIPBasic_Common", "802-1pMark", setValue);
	return 0;
}		
...
int8 getSIPRegisterRetryIntervalValue(char** value, ParameterType* type, DeviceNode*  deviceNode)
{
	...
	tcapi_get("VoIPBasic_Common", "RegisterRetryInterval", tempValue);
	if(!strlen(tempValue))
	{
		strcpy(tempValue, "0");
	}
...
#if defined(TCSUPPORT_ECN_SIP) || defined(TCSUPPORT_ECN_MEGACO) ||  defined(TCSUPPORT_CT_FJ)
...
#if defined(TCSUPPORT_CT_PON_NX) || defined(TCSUPPORT_CT_PON_SN)
