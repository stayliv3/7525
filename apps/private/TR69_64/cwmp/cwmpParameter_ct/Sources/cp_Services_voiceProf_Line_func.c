...
int8 getLineSubsUAValue(char** value, ParameterType* type, DeviceNode*  deviceNode)
{
	char tempValue[10]= {0};
	int iVal = 0;
	
	tcapi_get("VoIPAdvanced_Common", "SubscribeType", tempValue);
	if(tempValue[0] != '\n')
	{
		iVal = atoi(tempValue);
...
int8 setLineSubsUAValue(void* value, struct DeviceNode*  deviceNode)
{
	uint8 iReset;
	char tempValue[10] = {0};
	int iVal = 0;
	...
	tcapi_set("VoIPAdvanced_Common", "SubscribeType", tempValue);
	tcapi_commit("VoIPAdvanced_Common");
	
	return 0;	
}

int8 getLineSubsUAAttribute(uint8* notification, AccessEntry** acl, DeviceNode* deviceNode)
{
	uint8  attribute;
	int8  result;
	int instance = 0;
...
int8 setLineSubsUAAttribute(uint8 notificationChange, uint8 notification, uint8 accessListChange, AccessEntry* acl, struct DeviceNode*  deviceNode)
{
	uint8*  attribute = NULL;
	int8  result;
	int instance = 0;
...
int8 getLineSubsRegValue(char** value, ParameterType* type, DeviceNode*  deviceNode)
{
	char tempValue[10]= {0};
	int iVal = 0;
...
int8 setLineSubsRegValue(void* value, struct DeviceNode*  deviceNode)
{
	uint8 iReset;
	char tempValue[10] = {0};
	int iVal = 0;
...
int8 getLineSubsRegAttribute(uint8* notification, AccessEntry** acl, DeviceNode* deviceNode)
{
	uint8  attribute;
	int8  result;
	int instance = 0;
...
int8 setLineSubsRegAttribute(uint8 notificationChange, uint8 notification, uint8 accessListChange, AccessEntry* acl, struct DeviceNode*  deviceNode)
{
	uint8*  attribute = NULL;
	int8  result;
	int instance = 0;
...
static uint32 lineErrorStatusReasonGet(int instance)
{
	...
	char   tempValue[16] = {0};
	uint32 tempU32 = 0;
	char   path[] = "/proc/tc3162/pcmlastregerror";
	int    offset = 0;
...
int8 setLineLastRegisterErrorReasonValue(void* value, struct DeviceNode*  deviceNode)
{
	...
	tempU32: bit31~bit24  |  bit23~bit16  |  bit15~bit8  | bit7~bit0
				reserved         reserved        Line2            Line1
	*/		
	sscanf(tempValue,"%lu",&u32Temp);
	u8Temp = (uint8)valueInt;
	u32Temp &= ~(0xff << (instance << 3));
	u32Temp |= (u8Temp << (instance << 3));
	
	sprintf(temp,"echo %d > /proc/tc3162/pcmlastregerror",u32Temp);
	system(temp);	
...
int8 setconference_uriValue(void* value, struct DeviceNode*  deviceNode)
{
	...
	sprintf(nodeName, "VoIPIMS_Entry%d",  instance-1); 
