...
/*
**********************************************************************************
vlan filter  entry ioctl data structure.
**********************************************************************************
*/
typedef struct gponVlanFilterIoctl_s{
	uint16 port; //LAN port 0~3, ANI port: 0~31
	uint8 portType;//0:LAN port, 1:ANI port
	uint8 type; //bit0::set for untagged frame, bit1:set for tagged frame
	uint8 untaggedAction; //0:bridge, 1:discard
	/*
		0:bridge, 
		1:discard, 		
	...
	*/
	uint8 taggedAction;;
	uint8 maxValidVlanListNum; //max valid number in vlan list.
	uint8 cleanFlag;
	uint16 vlanList[MAX_GPON_VLAN_FILTER_LIST];
