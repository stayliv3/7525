...
#ifdef TCSUPPORT_EPON_OAM_CUC
#define CUC_VER_LIST_OFFSET 1
#else
#define CUC_VER_LIST_OFFSET 0
#endif

u_char gOuiCtcVerList[] =
{
#ifdef TCSUPPORT_EPON_OAM_CUC
	0xc1,
	0xc2,
...
/*-----------------判断CTC版本是否支持--------------------*/
int isSupportVersion(u_char version)
{
	int count = sizeof(gOuiCtcVerList) / sizeof(gOuiCtcVerList[0]);
	int i;
	for (i = 0; i < count; i++){
		if (version == gOuiCtcVerList[i])
			return TRUE;
	}
	return FALSE;
}
...
CTC_DSCVY_FSM:	
	...
	switch(gCtcHandler.dscvryState[llid]){
		...
		{
			int verCnt = sizeof(gOuiCtcVerList) / sizeof(gOuiCtcVerList[0]);
			switch (ctcVer){
			        #ifdef TCSUPPORT_EPON_OAM_CUC
			        case 0xc3:
			                break;
			        #endif
				case 0x30:
				        verCnt -= CUC_VER_LIST_OFFSET;
					break;
				case 0x21:
					verCnt -= 1 + CUC_VER_LIST_OFFSET;
				break;
				case 0x20:
					verCnt -= 2 + CUC_VER_LIST_OFFSET;
				break;
				case 0x13:
					verCnt -= 3 + CUC_VER_LIST_OFFSET;
				break;
				case 0x01:
					verCnt -= 4 + CUC_VER_LIST_OFFSET;
				break;
				#ifdef TCSUPPORT_EPON_OAM_CUC
				case 0xc1:
					verCnt -= 6;
				...
				#endif
				default:
					verCnt -= 1 + CUC_VER_LIST_OFFSET;// highest version is 0x21
			}
		...
		case DS_SEND_EXT_INFO:
		{
			...
			*outLen    = len;
			...
			if ( isSupportVersion(pInInfoHdr->version) && pInInfoHdr->extSupport == 0x01
