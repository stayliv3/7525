...
get_codec_mode(u_int * pCodec, u_char idx)
{
    ...
    {
        "G711AlawPriority","G729Priority",
        "G711UlawPriority","G723Priority",
        "G726_16Priority" ,"G726_32Priority"
    };

    enum {NAME_LEN = 35, BUFF_LEN = 10};
    char node[NAME_LEN];
    char buff[BUFF_LEN];
    ...
    snprintf(node, NAME_LEN, VOIPADVANCED_ENTRY, idx);
        
    for (; i < ARRAY_SIZE(codecs); ++i){
        if ( 0 != oam_tcapi_getul(node, codecs[i], buff, &tmp) ){
            OAM_FUNC_TRACE_LEAVE;
            return -1;
        }
...
db_voip_conf_update_pots_status(OamCtcDB_t *pDB)
{
    ...
    for (; portIdx < OAM_VOIP_PORT_NUM_MAX; ++ portIdx ){
        ...
        if( 0 !=  db_voip_conf_update_single_pots_status(pPort, portIdx)){
            *pFlag = VOIP_NODE_INVALID;
            OAM_FUNC_TRACE_LEAVE;
            return;
        }
