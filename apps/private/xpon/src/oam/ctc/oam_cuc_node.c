...
{
#define ETH_PORT_MODE_10HF	0x01
#define ETH_PORT_MODE_10FL	0x02
#define ETH_PORT_MODE_100HF	0x03	
#define ETH_PORT_MODE_100FL	0x04
#define ETH_PORT_MODE_1G		0x05
	u_char portId = sysObjIdx(pObj, PORT_ETHER);
	u_char state;
#if defined(TCSUPPORT_CUC)
	u_char mode;
	...
	if (portId == PORT_ERROR)
		return VAR_BAD_PARAMETERS;
#if defined(TCSUPPORT_CUC)
	macMT7530GetAutoDetection(&mode , portId);
	state = findEponModeByGPON(mode);
#else
	state = macMT7530GetConfigurationStatus(portId);
	switch (state){
		case 0x01: // 10BaseT full duplex
			state = ETH_PORT_MODE_10FL;
		break;
		case 0x02: // 100BaseT full duplex
			state = ETH_PORT_MODE_100FL;
		break;
		case 0x03: // 1000BaseT full duplex
			state = ETH_PORT_MODE_1G;
		break;
		case 0x11: // 10BaseT half duplex
			state = ETH_PORT_MODE_10HF;
		break;
		case 0x12: // 100BaseT half duplex
			state = ETH_PORT_MODE_100HF;
		break;
		case 0x13: // 1000BaseT half duplex
			state = ETH_PORT_MODE_1G;
		break;
		default: // 10BaseT full duplex
			state = ETH_PORT_MODE_100FL;		
	}
#endif
...
#if defined(TCSUPPORT_CUC)
	portMode = findGponModeByEPON(mode);
	if (SUCCESS != macMT7530SetAutoDetection(portMode,portId))
		return VAR_NO_RESOURCE;
#else
	if (SUCCESS != macMT7530SetPortSpeedMode(portId, mode))
		return VAR_NO_RESOURCE;
#endif
