...
int oam_ctc_db_init(OamCtcDB_t *pDB){
	...
	pDB->force_ver = 0x30;
	memset(buf, 0, sizeof(buf));
	if (TCAPI_SUCCESS == tcapi_get("Oam_CTC", "ForceVer", buf)){
		pDB->force_ver = atoi(buf);
	}
	...
	if (isMT7520S(cpu_type)){
#if defined(TCSUPPORT_CT_C5_HEN_SFU)
		pDB->dev_info.fe_num = 1;	
#else
		pDB->dev_info.ge_num = 1;
#endif
	}else if (isMT7520F(cpu_type)){
		pDB->dev_info.voip_num = 2;
		pDB->dev_info.ge_num = 1;
		pDB->dev_info.fe_num = 3;		
	}else if (isMT7520G(cpu_type)){
		pDB->dev_info.voip_num = 2;
		pDB->dev_info.ge_num = 4;		
	}else if (isMT7525F(cpu_type)){
		pDB->dev_info.voip_num = 2;
		pDB->dev_info.usb_num = 2;
		pDB->dev_info.wlan_num = 2;
		pDB->dev_info.ge_num = 1;		
		pDB->dev_info.fe_num = 3;
	}else if (isMT7525G(cpu_type)){
		pDB->dev_info.voip_num = 2;
		pDB->dev_info.usb_num = 2;
		pDB->dev_info.wlan_num = 2;
		pDB->dev_info.ge_num = 4;
	}else if (isEN7521(cpu_type)){
		/* Need to confirm */
		pDB->dev_info.voip_num = 2;
		pDB->dev_info.usb_num = 2;
		pDB->dev_info.wlan_num = 2;
...
int oam_ctc_db_print(OamCtcDB_t *pDB){
	...
	dprintf("[CPU: %s, OUI: %06X]\n", (isMT7520S(pDB->cpu_type)?"MT7520S":\
								  ...
								  "Unkown"))))), pDB->oui);
	dprintf("[DBG Level: %u, ForceVer: %02x]\n", pDB->debug_level, pDB->force_ver);
