...
{"rtp errors", 			4, 0, NO_TCA, 	NULL},
{"packet loss", 		4, 0, OMCI_TCA_ID_RTP_PACKET_LOSS, 	NULL},
{"maximum jitter",      4, 0, OMCI_TCA_ID_RTP_PACKET_JITTER,   NULL},
{"maximum time between RTCP", 		4, 0, NO_TCA, 	NULL},
{"no RTCP packet",      4, 0, OMCI_TCA_ID_RTP_NO_RTCP_PACKET,   NULL},
{"timeout",             4, 0, OMCI_TCA_ID_RTP_TIMEOUT,   NULL},
...
int pmmgrPmInitForRTPDataPM(pmmgrPmMe_t *me_p)
{
    ...
    attri_p++;
    attri_p->offset = (uint8*)&pm.NoRTCPPacket - (uint8*)&pm.RtpErrors;

    attri_p++;
    attri_p->offset = (uint8*)&pm.Timeout - (uint8*)&pm.RtpErrors;
...
int getAllDataFromApiForRTPDataPM(char *data, uint16 portId)
{
    ...
    sprintf(nodeName[1], "%s%x", "Entry", portId);  
    ...
    if(getAttrValue(top, nodeName, OMCI_RTP_PM_MaxTimeBetRTCP, tempBuffer) != TCAPI_PROCESS_OK){
        ret = 0;
        pmmgrPrintf(PMMGR_DEBUG_TRACE, "\r\n getAllDataFromApiForRTPDataPM->RTCP ,  fail");
        goto end;
    }
    ...
    pmmgrPrintf(PMMGR_DEBUG_TRACE,"Enter getAllDataFromApiForRTPDataPM pm->NoRTCPPacket = %lu\n", pm->NoRTCPPacket);
    pmmgrPrintf(PMMGR_DEBUG_TRACE,"Enter getAllDataFromApiForRTPDataPM pm->Timeout = %lu\n", pm->Timeout);
