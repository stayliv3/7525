...
int line_error_restartsip_timer_cnt = 0;
extern int sipStatusFlag;
extern int VOIPLineNum;
static void restart_sip_when_sip_down(void);
...
static inline void check_omci_need_restart(){
    ...
    char fe_reset_happened=0, roger_flag=0;
    ...
    fp1 = fopen("/proc/xpon/rogue_mode", "r");
    if(fp1){
        roger_flag = fgetc(fp1);
        fclose(fp1);
    }
    if((fe_reset_happened =='1') && (roger_flag =='0')) {
...
start_cc(void* unused)
{
	...
	while(1){
		...
		if(count_changeCongestionMode<45)
		{
			changeCongestionMode();
			count_changeCongestionMode++;
			//tcdbg_printf("\nchangeCongestionMode() has runed %d times!!!\n",count_changeCongestionMode);
		}
#endif
#endif
...
void check_led_switch_button(int pon_stats)
{
	...
	if(fd != -1)
	{
		...
		if(ret)
		{
			...
			if(0 != btnStats)	               // led switch button is pressed
			{
				if(LED_SWITCH_ON == oldStatus)
				{
					system("sys led off");
...
#if defined(TCSUPPORT_CT_PON_SC) || defined(TCSUPPORT_CT_PON_C9) || defined(TCSUPPORT_CT_PON_CY_JX)
	PHASE1_SEC = 10;
	PHASE2_SEC = 60;
#endif
...
static void restart_sip_when_sip_down()
{
	FILE *fp = NULL;
	char cmdbuf[128] = {0};
	char sip_pid_tmp[100] = {0};
        char FlagFwUp_s[20] = {0};
	fp = fopen("tmp/sipclient.pid","r");
	if(fp == NULL){
		/*get sipclient process pid*/
		sprintf(cmdbuf, "pidof %s > %s", "sipclient", "tmp/sipclient.pid");
		system(cmdbuf);  
		fp = fopen("tmp/sipclient.pid", "r");
	}
	if(fp == NULL){
	...
	fclose(fp);
	unlink("tmp/sipclient.pid");
	tcapi_get("System_Entry", "upgrade_status", FlagFwUp_s);
	if((atoi(sip_pid_tmp) == 0)&&(0 != strcmp(FlagFwUp_s,"true"))){  /*if sip_pid_tmp == 0, that's mean need restart sipclient*/
...
static void restart_sip_when_line_unreg()
{
	...
	int regRetryTime = 0;
	int line_no_unreg_flag = 0;/* 0:none of line is unregistered;1:some of line is not untegistered*/
	char FlagFwUp_s[20] = {0};
	...
	for(j=0 ; j<VOIPLineNum ; j++){
		...
		if( TCAPI_PROCESS_OK == tcapi_get(nodeName, "Enable", lineEnable)/*make sure line is enable*/
			...
			&& (strcmp(strRegStatus,"Unregistered") == 0)){
				line_no_unreg_flag = 1;
				if(line_error_restartsip_timer_cnt <= regRetryTime){
					line_error_restartsip_timer_cnt++;
				}
				else{
					if(0 != strcmp(FlagFwUp_s,"true")){
					    tcdbg_printf("\nrestart sipclient for line error!!\n");
					    system("killall -9 sipclient");
					    system("/userfs/bin/sipclient &");
					    sipStatusFlag = 1;
					    line_error_restartsip_timer_cnt = 0;
				        }
				}
				break;
			}						
	...
	if(line_no_unreg_flag == 0){/*none of line is in unregistered status*/
		line_error_restartsip_timer_cnt = 0;
	}
	return;
}
...
int voipStatUpgrade(void)
{
	...
	for(i = (valid_if_num - 1); i > -1; i--)
	{
		...
		if( TCAPI_PROCESS_OK == tcapi_get(nodeName, "WanMode", str_wanmode) &&
...
static int voip_simulate_send_dtmf(int line_id)
{
	...
	{		
		...
		if(getSipStatus() == 1) {
			...
			voipCmdSend("127.0.0.1",3555,testCmd);
#if defined(TCSUPPORT_ECN_SIP)
		}
#endif
#endif
...
static int voip_simulate_result_deal(int line_id)
{
	...
	if(SIMULATE_CALLER == gVoipSimulateState[line_id].testType) {
		switch (gVoipSimulateState[line_id].line_state) {
			...
			case SIMULATE_STATUS_CONNECTED:
				simulate_result_set(line_id, SIMULATE_CALLER_OFFHOOK_RELEASE, "Fail");
				goto SIMULATE_TEST_END;

				break;
			case SIMULATE_STATUS_DIAL_TONE:
				simulate_result_set(line_id, SIMULATE_CALLER_AFTER_DIAL_REALEASE, "Fail");
				goto SIMULATE_TEST_END;

				break;
			case SIMULATE_STATUS_RINGING_BACK:
				simulate_result_set(line_id, SIMULATE_CALLER_NO_ANSWER, "Fail");
				goto SIMULATE_TEST_END;
		...
		switch (gVoipSimulateState[line_id].line_state) {
			case SIMULATE_STATUS_IDLE:
				...
				if (!strcmp (tmpStr, "Idle"))
					simulate_result_set(line_id, SIMULATE_CALLED_NO_INCOMING_CALL, "Fail");
				else if (!strcmp (tmpStr, "Ringing"))
					simulate_result_set(line_id, SIMULATE_CALLED_NO_ANSWER, "Fail");
				else 
					simulate_result_set(line_id, SIMULATE_CALLED_NO_INCOMING_CALL, "Fail");
				goto SIMULATE_TEST_END;
				break;
			...
			case SIMULATE_STATUS_OFFHOOK:
			case SIMULATE_STATUS_CONNECTED:
				simulate_result_set(line_id, SIMULATE_CALLED_OFFHOOK_RELEASE, "Fail");
				goto SIMULATE_TEST_END;
				break;
	...
	if (TCAPI_PROCESS_OK == retval)
	{
		...
		if ((!strcmp(gVoipSimulateState[line_id].confirmEnable, "Yes")
			...
			simulate_result_set(line_id, SIMULATE_NO_ERROR, "Success");
		}
		else{
			simulate_result_set(line_id, SIMULATE_CALLER_KEY_ERROR, "Fail");
		}
...
static int voip_simulate_block_deal(int line_id)
{
	...
	if (SIMULATE_CALLED == gVoipSimulateState[line_id].testType){
		...
		simulate_result_set(line_id, SIMULATE_CALLER_NO_DIAL_TONE, "Fail");
	}
	simulate_status_set(line_id, SIMULATE_STATUS_TEST_END);
	simulate_testState_set(line_id, "Complete");
	voip_simulate_onhook(line_id);	
