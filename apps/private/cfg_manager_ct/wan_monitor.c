	...
	if(needCommit == 1){
		...
		if (!strcmp(xponModeBuf, "EPON")) { /* only for epon product */
			...
			if (g_adsl_state == ADSL_STATE_DOWN && strcmp(linkStat, "1")) { /* only for epon mode */
				...
				setXponModeSwitch(XPON_EPON_MODE, 0); /* stop first */
...
int updateVoIP(mxml_node_t *top,char *if_name,int if_index,int if_state){
	...
	if(if_state == IF_STATE_UP){
		if(strcmp(bindIf,if_name)){
			...
			ret=voipCmdSend("127.0.0.1",3555,cmdBuf);
			if(ret==-1){
				goto restart_mgapp;
			}
		}
		...
		if(flag == 0){
			...
			ret=voipCmdSend("127.0.0.1",3555,"callCmd {test.Restart}");
			if(ret==-1){
				goto restart_mgapp;
			}
		}
		...
		ret=voipCmdSend("127.0.0.1",3555,"callCmd {test.wanDown}");
		if(ret==-1){
			goto restart_mgapp;
		}
	}
