	...
	if (isValidChanId(pVal)) {
		...
		if (isValidNum(pVal, ~(MAX32 >> 1), MAX32 >> 1)) {
			...
			if (strlen(pVal) > 0) {
				...
				#else
				if(EXEC_SUCCESS != adamChanPlayTone(DSP_ID, ch, &toneList,rpt)){
					printf("Error: adamChanPlayTone failed! \n");
					return;
				}
				#endif
...
void cmdStrmConfig(const char *inStr) {
	...
	if (isValidStrmId(pVal, &ch, &st)) {
        ...
        #ifdef AMR_CODEC
		printf("redundant_pt: %d,g726_pt: %d,amr_pt: %d\n",config.payloadType[CODEC_REDUNDANT],config.payloadType[CODEC_G726],config.payloadType[CODEC_AMR]);
        #else        
		printf("redundant_pt: %d,g726_pt: %d\n",config.payloadType[CODEC_REDUNDANT],config.payloadType[CODEC_G726]);
        #endif
		...
		if(tmpSlicType == TYPE_ZARLINK)
			{
					...
					{					
						...
						{	
							  /* Test results for the test LT_TID_LINE_V */
								...
								pResultTemp=(LtLineVResultType*)lineTest.lineTestData;
						...
						case EVA_LT_TID_ROH:
						{
							  ...
							  pResultTemp=(LtRohResultType*)lineTest.lineTestData;
						...
						{				
								...
								pResultTemp=(LtRingersResultType*)lineTest.lineTestData;
						...
						case EVA_LT_TID_RES_FLT:
						{
							  ...
							  pResultTemp=(LtResFltResultType*)lineTest.lineTestData;
						...
						case EVA_LT_TID_DC_FEED_ST:
						{
							  ...
							  pResultTemp=(LtDcFeedSTResultType*)lineTest.lineTestData;
						...
						case EVA_LT_TID_DC_VOLTAGE:
						{
							  ...
							  pResultTemp=(LtDcVoltageResultType*)lineTest.lineTestData;
						...
						case EVA_LT_TID_RINGING_ST:
						{
							  ...
							  pResultTemp=(LtRingingSTResultType*)lineTest.lineTestData;
				...
				//				  VpInitRing(pLCtx, RING_CAD_STD, VP_PTABLE_NULL);
						...
						case EVA_LT_TID_ON_OFF_HOOK_ST:
						{
							  ...
							  pResultTemp=(LtOnOffHookSTResultType*)lineTest.lineTestData;
						...
						{	
							  ...
							  pResultTemp=(LtReadBatCondResultType*)lineTest.lineTestData;
						...
						{	
							  ...
							  pResultTemp=(LtPreLineVResultType*)lineTest.lineTestData;
						...
						{	
							  ...
							  pResultTemp=(LtCapResultType*)lineTest.lineTestData;
							  printf("	 fltMask	= %d\n" ,pResultTemp->fltMask	  )  ;	
							  printf("	 ctg		= %ld\n" , pResultTemp->ctg 	  )  ;
							  printf("	 crg		= %ld\n" , pResultTemp->crg 	  )  ;
							  printf("	 ctr		= %ld\n" , pResultTemp->ctr 	  )  ;	
						...
						{	
							  ...
							  pResultTemp=(LtRdLoopCondResultType*)lineTest.lineTestData;
				...
				{	ProSLICMLTType *pProSLICMLT;
					pProSLICMLT=(ProSLICMLTType*)lineTest.lineTestData;
					switch (lineTest.lineTestId)
					{					
...
void GetConfigDynamicPtOfCodec(const char *inStr,strmConfig_t *config)
{
	...
	if (strlen(pVal) > 0 && isValidNum(pVal, 96, 127)) 
    {
		config->payloadType[CODEC_G726]= atoi(pVal);
	}
