...
json_t* GetPhyInterConStatus(json_t* req_root, json_t* req_para)
{
	...
	if(ret == -1){
		...
		if(JSON_OK != ParameterAddNewJSONChild(reply_para, PARA_LAN3STATUS, LanPortConStatus[2])){
			mobile_printf(MOBILE_INFO,"%s:ParameterAddNewJSONChild Err!...\n",__FUNCTION__);
			goto err;
		}
		...
		if(JSON_OK != ParameterAddJSONChild(reply_para, PARA_WANSTATUS, TCSUPPORT_CT_2PORTS_VAL ? JSON_NULL : JSON_STRING, WanPortConStatus)){
			mobile_printf(MOBILE_INFO,"%s:ParameterAddNewJSONChild Err!...\n",__FUNCTION__);
			goto err;
		}
