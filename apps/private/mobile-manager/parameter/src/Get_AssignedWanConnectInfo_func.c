...
json_t* Get_AssignedWanConnectInfo(json_t* req_root, json_t* req_para)
{
	...
	char cur_servicelist[64] = {0}, cur_wanmode[16] = {0}, cur_vid[6] = {0}, cur_vid2[6] = {0}, cur_8021p[6] = {0};
	...
	if(existflag == 0){
		...
		if(JSON_OK != ParameterAddNewJSONChild(reply_para, PARA_VLANID , cur_vid2)){
			mobile_printf(MOBILE_INFO,"%s:ParameterAddNewJSONChild Err!...\n",__FUNCTION__);
			goto err;
		}
